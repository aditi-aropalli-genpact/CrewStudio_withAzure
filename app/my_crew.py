...
class MyCrew:
    ...
    def update_tasks(self, task_id, user_id):
        selected_tasks_ids = task_id
        self.tasks = [task for task in db_utils.load_tasks(user_id) if task.id in selected_tasks_ids and task.agent.id in [agent.id for agent in self.agents]]
...
async def create_crew(crew_data: CrewCreate):
    ...
    crew = {
        'name': crew_data.name,  # No "id" field here
        'process': crew_data.process,
        'verbose': crew_data.verbose,
        'agent_ids': crew_data.agent_ids,  # Ensure this is a list of IDs
        'task_ids': crew_data.task_ids,  # Ensure this is a list of IDs
    ...
    db_utils.save_entity('crew', crew_id, crew, user_id)
