...
#             self.session_state['results'] = load_results(user_id)
...
def api_run_crew(crew_name: str,token_payload: dict = Depends(verify_token)):
        # Extract user_id from token payload  
    user_id = token_payload.get('OHR') 

    if not user_id:  
        raise HTTPException(status_code=401, detail='User ID not found in token')  
    selected_crew = load_crew_by_name(crew_name, user_id)
...
def api_stop_crew(token_payload: dict = Depends(verify_token)):
...
def api_get_result(token_payload: dict = Depends(verify_token)):
...
def api_clear_console(token_payload: dict = Depends(verify_token)):
